...
# Tests for generating expressions in Polish Notation (PN)
class TestPolishNotation(unittest.TestCase):
    def test_addition(self):
        tree = Tree("1+1")
        self.assertEqual(tree.toPolishNotation(), "+ 1 1")
        tree = Tree("1+1+1")
        # TODO: This should probably be "+ 1 1 1" but it isn't because the
        #       syntax trees we generate are binary
        self.assertEqual(tree.toPolishNotation(), "+ 1 + 1 1")

    def test_subtraction(self):
        tree = Tree("2-1")
        self.assertEqual(tree.toPolishNotation(), "- 2 1")

    def test_multiplication(self):
        tree = Tree("2*3")
        self.assertEqual(tree.toPolishNotation(), "* 2 3")
    ...
    def test_exponents(self):
        tree = Tree("3^5")
        self.assertEqual(tree.toPolishNotation(), "^ 3 5")

    def test_factorial(self):
        tree = Tree("5!")
        self.assertEqual(tree.toPolishNotation(), "! 5")
        tree = Tree("(4+1)!")
        self.assertEqual(tree.toPolishNotation(), "! + 4 1")

    def test_order_of_operations(self):
        tree = Tree("3 + 4 * 5")
        self.assertEqual(tree.toPolishNotation(), "+ 3 * 4 5")

        tree = Tree("4 * 5 + 3")
        self.assertEqual(tree.toPolishNotation(), "+ * 4 5 3")

    def test_parentheses(self):
        tree = Tree("(3 + 4) * (5 + 6)")
        self.assertEqual(tree.toPolishNotation(), "* + 3 4 + 5 6")
